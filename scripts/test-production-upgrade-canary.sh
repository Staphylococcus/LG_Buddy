...
INSTALLED_DESKTOP_ENTRY="$INSTALL_ROOT/usr/share/applications/io.github.staphylococcus.LGBuddy.desktop"
...
[ -x "$INSTALLED_GUI" ] || fail "Published upgrade did not install the GUI."
[ "$("$INSTALLED_GUI" --version)" = "$EXPECTED_VERSION_OUTPUT" ] || fail "Installed GUI identity does not match the published canary target."
[ -f "$INSTALLED_ICON" ] || fail "Published upgrade did not install the application icon."
grep -F -x -q 'Icon=io.github.staphylococcus.LGBuddy' "$INSTALLED_DESKTOP_ENTRY" || fail "Published upgrade did not install the named application icon entry."
[ ! -e "$LEGACY_DESKTOP_ENTRY" ] || fail "Published upgrade left the legacy desktop entry installed."
